...
printInfo <- function(string, inclusionTerms = TRUE, exclusionTerms = TRUE, tabular = 'single') {
  ...
  if(strLength == 3) { # categoría
    query <- categories %>% filter(category == string)

    if(tabular == 'single') {
      query <- query %>%
        filter(term == 'Canonical') %>%
        mutate(subcategory = NA) %>%
        select(category, subcategory, disease = categoryTitle)
    ...
    if(tabular == 'simple') return(query)
      ...
      if(hasInclusion) inclusions <- query %>% filter(term == 'Inclusion') %>% pull(categoryTitle)
      if(hasInclusion) exclusions <- query %>% filter(term == 'Exclusion') %>% pull(categoryTitle)
      ...
      fprintf(paste('-----------------------------------------------------------------------------',
              ...
              filter(query, term == 'Canonical')$categoryTitle,
    ...
    queryCategory <- categories %>% filter(category == str_sub(string, 1, 3)) %>%
      filter(term == 'Canonical') %>% pull(categoryTitle)
